...
To update the database use this command from the Web project folder (replace `Sample` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Sample.Infrastructure/Sample.Infrastructure.csproj -s Sample.Web.csproj
```
